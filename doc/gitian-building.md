...
*Setup instructions for a gitian build of BitGreen using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitGreen
...
- [Building BitGreen](#building-bitgreen)
...
Clone the git repositories for bitgreen and gitian and then checkout the bitgreen version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitgreen repository
...
To build BitGreen (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitgreen repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitgreen/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitgreen-official/bitgreen
...
For example:
```bash
URL=https://github.com/konez2k/bitgreen.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitgreen=${COMMIT} --url bitgreen=${URL} ../bitgreen/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitgreen=${COMMIT} --url bitgreen=${URL} ../bitgreen/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitgreen=${COMMIT} --url bitgreen=${URL} ../bitgreen/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitgreen-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitgreen-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitgreen-build.assert
```
...
[bitgreen/gitian.sigs](https://github.com/bitgreen/gitian.sigs/) repository, or if that's not possible to create a pull
request.
